...
sq(1, "Envia el siguiente mensaje con tema solicitud de cambio de grupo a master, con texto quiero cambiarme de grupo reducido de SI5 a SI2").
sq(2, "Dame de alta en faitic").
sq(3, "Tu master es brais fontan").
sq(4, "Soy Paco Perez").
sq(5, "Dame de alta en faitic").
sq(6, "Hardware de Aplicacion especifica").
sq(7, "Solicita un cambio de grupo reducido").
sq(8," De Sistemas Inteligentes").
sq(9," A el grupo SI_2").
sq(10," Realiza la reserva de un seminario").
sq(11," Para manhana a las 10:00").
sq(12," Si, el seminario 23").
sq(13," Quiero solicitar practicas en empresas").
sq(14," Quiero realizar 150 horas").
sq(15," En la empresa COASA").
sq(16," Solicita la defensa de mi TFG").
sq(17," Solicita la asignacion del TFG Simulacion de campos de luz avanzados").
sq(18," Maria Encarnacion Gonzalez Rufino").
sq(19," Solicita la defensa de mi TFG").
sq(20," Quiero solicitar la evaluacion por compensacion de Estadisitica").
sq(21, "Crea el set empresas").
sq(22, "Anhade el valor Inditex a el set empresas").
sq(23, "Crea el map evaluacion").
sq(24, "Anhade la relacion LC tiene 8.5 a el map evaluacion").
sq(25, "Crea el fichero mificherodeprueba").
sq(26, "Anhade la linea texto de prueba a el fichero mificherodeprueba").
...
+!chooseb(N,Answer) : dosq(N,_) <- ?sq(New,Answer); -sq(New,Answer); +dosq(New,Answer).
+!chooseb(N,Answer) : not dosq(N,_) & sq(N,Answer) <- -sq(N,Answer); +dosq(N,Answer).
+!chooseb(0,Answer) : sq(N,Answer) <- -sq(N,Answer); +dosq(N,Answer).

+!level0 <-
	for (.range(I,1,5)) {
		Sel = math.round(math.random(17))+1;
		.println;
		.println(" ===============================> La regla tq elegida es la: ", Sel," <===============================");
		.println;
		!select(Sel,Ans);
		.println;
...
+!level1 <-
	for (.range(I,1,5)) {
		Sel = math.round(math.random(17))+1;
		.println;
		.println(" ===============================> La regla aq elegida es la: ", Sel," <===============================");
		!choose(Sel,Ans);
		chat(Ans);
		.wait(2000);
		.println;
...
+!level2 <-
	for (.range(I,1,5)) {
		...
		.println;
		.println(" ===============================> La regla ntq elegida es la: ", Sel," <===============================");
		!selectb(New,Ans);
		chat(Ans);
		.wait(2000);
	}.
...
+!level3 <-
	for (.range(I,1,26)) {
		Sel = I;
		.println;
		.println(" ===============================> La regla sq elegida es la: ", Sel," <===============================");
		!chooseb(Sel,Ans);
		chat(Ans);
		.wait(3000);
	}.
...
+answer(Answer) : not filter(Answer, Tag, Msg) & numAnswer(N) <- 
	...
	.println("=======================================================================");
	.println;
	.println("Student: Acabo de recibir del bot la contestacion: ", Answer);
	.println;
	.println("=======================================================================");
	-answer(Answer);
	.wait(1000);
	if(.substring("Quieres que te hable sobre",Answer)){
		chat("Si");
	}.

+answer(Answer) : filter(Answer, Tag, Msg) & numAnswer(N) <- -answer(Answer).
